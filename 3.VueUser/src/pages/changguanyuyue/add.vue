...
<div :style='{"width":"100%","padding":"30px calc((100% - 1200px)/2) 40px","margin":"0px auto","position":"relative","background":"#fff"}'>
    ...
    >
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="场馆名称" prop="changguanmingcheng">
            <el-input v-model="ruleForm.changguanmingcheng" 
                placeholder="场馆名称" clearable :disabled=" false  ||ro.changguanmingcheng"></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="场地类型" prop="changdileixing">
            <el-input v-model="ruleForm.changdileixing" 
                placeholder="场地类型" clearable :disabled=" false  ||ro.changdileixing"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}'  label="场地数量" prop="changdishuliang">
            <el-select v-model="ruleForm.changdishuliang" placeholder="请选择场地数量" :disabled=" false  ||ro.changdishuliang" >
              <el-option
                  v-for="(item,index) in changdishuliangOptions"
                  :key="index"
                  :label="item"
                  :value="item">
              </el-option>
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="容纳人数" prop="rongnarenshu">
            <el-input v-model="ruleForm.rongnarenshu" 
                placeholder="容纳人数" clearable :disabled=" false  ||ro.rongnarenshu"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="预约时长" prop="yuyueshizhang">
            <el-input v-model.number="ruleForm.yuyueshizhang" 
                placeholder="预约时长" clearable :disabled=" false  ||ro.yuyueshizhang"></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="会员折扣" prop="huiyuanzhekou">
			<el-input-number v-model="ruleForm.huiyuanzhekou" placeholder="会员折扣" :disabled="ro.huiyuanzhekou"></el-input-number>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="实付金额" prop="shifujine">
              <el-input v-model="shifujine" placeholder="实付金额" disabled></el-input>
          </el-form-item>
...
<script>
  export default {
    ...
    computed: {
        ...
        shifujine:{
            get: function () {
                return 1*this.ruleForm.yuyueshizhang*this.ruleForm.jiage*this.ruleForm.huiyuanzhekou
            }
        },
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            this.ruleForm.changdishuliang=Number('1') ;
            this.ruleForm.changdishuliang = 0;
            this.ro.changdishuliang = false;
        }else if(type=='edit'){
			this.info()
		}
        // 获取用户信息
        ...
        this.changdishuliangOptions = "1".split(',')
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`changguanyuyue/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
      ...
      onSubmit() {
		this.ruleForm.ispay = '未支付'
			...
			if(!this.ruleForm.changdishuliang){
				this.$message.error("场地数量不能为空");
				return
			}
			var obj = JSON.parse(localStorage.getItem('crossObj'));
			var table = localStorage.getItem('crossTable');
			obj.changdishuliang = obj.changdishuliang - this.ruleForm.changdishuliang
			if(obj.changdishuliang<0){
				this.$message.error("场地数量不足");
				return
			}
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('changguanyuyue/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								obj.changdishuliang = parseFloat(obj.changdishuliang) - parseFloat(this.ruleForm.changdishuliang)

								this.$http.post(table+`/update`,obj).then(res => {});
								this.$http.post(`changguanyuyue/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
