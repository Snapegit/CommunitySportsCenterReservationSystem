			...
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"display":"block"}' >
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"none","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">教练姓名</label>
						<el-input v-model="searchForm.jiaolianxingming" placeholder="教练姓名" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"none","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">主教课程</label>
						<el-input v-model="searchForm.zhujiaokecheng" placeholder="主教课程" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('jiaolian','预约')" @click="yuyuejiaolianCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
							...
							<el-button class="view" v-if="isAuth('jiaolian','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"rgb(139, 154, 179)","display":"none","height":"40px"}'></span>
								查看评论
							</el-button>
...
import yuyuejiaolianCrossAddOrUpdate from "../yuyuejiaolian/add-or-update";
	export default {
		...
		methods: {
			...
			yuyuejiaolianCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.$storage.set('crossTable','jiaolian');
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.zhujiaokecheng!='' && this.searchForm.zhujiaokecheng!=undefined){
            params['zhujiaokecheng'] = '%' + this.searchForm.zhujiaokecheng + '%'
          }
	...
	this.$router.push({path:'/discussjiaolian',query:{refid:id}});
    },
    // 下载
    download(file){
