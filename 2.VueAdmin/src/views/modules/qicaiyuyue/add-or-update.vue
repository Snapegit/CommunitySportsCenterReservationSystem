	...
	<div class="addEdit-block" :style='{"width":"100%","padding":"50px 0 30px 0","margin":"0","overflow-y":"auto","background":"#fff","height":"auto"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="位置" prop="weizhi">
					<el-input v-model="ruleForm.weizhi" placeholder="位置" clearable  :readonly="ro.weizhi"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="位置" prop="weizhi">
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="会员折扣" prop="huiyuanzhekou">
					<el-input-number v-model="ruleForm.huiyuanzhekou" placeholder="会员折扣" :disabled="ro.huiyuanzhekou"></el-input-number>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="会员折扣" prop="huiyuanzhekou">
					<el-input v-model="ruleForm.huiyuanzhekou" placeholder="会员折扣" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="实付金额" prop="shifujine">
					<el-input v-model="shifujine" placeholder="实付金额" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.shifujine" label="实付金额" prop="shifujine">
					<el-input v-model="ruleForm.shifujine" placeholder="实付金额" readonly></el-input>
				</el-form-item>
...
export default {
	...
	computed: {
		...
		shifujine:{
			get: function () {
				return 1*this.ruleForm.jiage*this.ruleForm.huiyuanzhekou
			}
		},
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `qicaiyuyue/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.qicaiyuyueCrossAddOrUpdateFlag = false;
											this.parent.search();
